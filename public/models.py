...
class SimulationProperty(EntryData):
	...
	def get_as_html_code(self):
		pathParts = self.class_name.split('.')
		...
		lexer = MatlabLexer()
		lexer.add_filter(PropertyDefinitionFilter(property_name = self.property_name, tokentype=Token.Name.Variable)) 
		
		tokens = lexer.get_tokens(code)
			
		return pygments.format(tokens, PygmentsFormatter(linenos='inline', linenostep=1, style=PygmentsStyle, noclasses=True))
...
class PropertyDefinitionFilter(Filter):
	def __init__(self, **options):
		...
		self.property_name = options.get('property_name')
	
	def filter(self, lexer, stream):
		for ttype, value in stream:
			if value == self.property_name:
				ttype = Token.Name.Variable
			yield ttype, value
