...
def render_queryset_to_response(request = [], queryset = EmptyQuerySet(), models = [], templateFile = '', data = {}, species_wid=None):	
	...
	if format == 'html':
		...
		json['comments'] = 'Generated by %s on %s at %s' % ('WholeCellKB', now.isoformat(), settings.BASE_URL + reverse('public.views.exportData', kwargs={'species_wid': species.wid}))
		...
		comment = doc.createComment('\n%s WholeCellKB\nGenerated by %s on %s at %s\n%s %s %s\n' % (
			...
			settings.BASE_URL + reverse('public.views.exportData', kwargs={'species_wid': species.wid}),
